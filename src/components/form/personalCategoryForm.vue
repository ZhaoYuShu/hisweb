  ...
  <div class="rightForm">
    ...
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="deleteCustomerType" size="small">确 定</el-button>
        </span>
    </el-dialog>
    <!--表单-->
      ...
      <el-row>
        <el-col :span="24">
          <el-form-item label="名称" prop="name" required>
            <el-input v-model.number="ruleForm.name" placeholder="请输入人员类别名称"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="20">
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        name: [
          { required: true, message: '请输入人员类别名称', trigger: 'blur' }
        ]
      },
      length: '',
      saveDisabled: false,
  ...
  methods: {
    ...
    submitForm (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          http.addCustomerType(that.ruleForm).then(function (response) {
            ...
            if (response.status === 200 && response.data.result === '00000000') {
              ...
              http.customerTypeTree().then(function (response) {
                console.log(response);
                Bus.$emit("addCustomerTypeTree", response.data.data);
    ...
    deleteCustomerType () {
      ...
      http.deleteCustomerType(this.currentId).then(function (response) {
        console.log(response);
        if (response.status === 200 && response.data.result === '00000000') {
          that.$message({
            message: '删除人员类别成功！',
          ...
          http.customerTypeTree().then(function (response) {
            console.log(response);
            Bus.$emit("deleteCustomerTypeTree", response.data.data);
            that.length = response.data.data.length;
          }).catch(function (error) {
            console.log(error);
          });
    ...
    updateForm (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          console.log('submit!');
          http.updateCustomerType(that.ruleForm).then(function (response) {
            console.log(response);
            if (response.status === 200 && response.data.result === '00000000') {
              that.$message({
                message: '修改人员类别成功！',
              ...
              http.customerTypeTree().then(function (response) {
                console.log(response);
                Bus.$emit("updateCustomerTypeTree", response.data.data);
              }).catch(function (error) {
                console.log(error);
              });
            } else {
    ...
    getCode () {
      let that = this;
      http.customerTypeCode().then(function (response) {
        console.log(response);
        if (response.status === 200 && response.data.result === '00000000') {
    ...
    reloadTree () {
      ...
      http.customerTypeTree().then(response => {
        ...
        if (response.status === 200 && response.data.result === '00000000') {
          that.length = response.data.data.length;
          Bus.$emit("reloadCustomerTypeTree", response.data.data);
  ...
  mounted () {
    ...
    Bus.$on("saveDisabledPersonalCategory", (e) => {
      this.saveDisabled = e;
    });
    Bus.$on("deleteDisabledPersonalCategory", (e) => {
      this.deleteDisabled = e;
    });
    Bus.$on("updateDisabledPersonalCategory", (e) => {
